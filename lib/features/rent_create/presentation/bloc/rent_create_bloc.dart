...
class RentCreateBloc extends Bloc<RentCreateEvent, RentCreateState> {
  ...
  void _onNavigateToStep(NavigateToStepEvent event, Emitter<RentCreateState> emit) {
    if (event.stepIndex >= 0 && event.stepIndex < PropertyCreationStep.values.length) {
  ...
  void _onUpdatePropertyTitle(UpdatePropertyTitleEvent event, Emitter<RentCreateState> emit) {
    final updatedFormData = state.formData.copyWith(propertyTitle: event.title);
    emit(state.copyWith(formData: updatedFormData));
  }

  void _onUpdatePropertyType(UpdatePropertyTypeEvent event, Emitter<RentCreateState> emit) {
  ...
  void _onUpdateBedrooms(UpdateBedroomsEvent event, Emitter<RentCreateState> emit) {
    final updatedFormData = state.formData.copyWith(bedrooms: event.bedrooms);
    emit(state.copyWith(formData: updatedFormData));
  }

  void _onUpdateBathrooms(UpdateBathroomsEvent event, Emitter<RentCreateState> emit) {
    final updatedFormData = state.formData.copyWith(bathrooms: event.bathrooms);
    emit(state.copyWith(formData: updatedFormData));
  }

  void _onUpdateNumberOfBeds(UpdateNumberOfBedsEvent event, Emitter<RentCreateState> emit) {
    final updatedFormData = state.formData.copyWith(numberOfBeds: event.numberOfBeds);
    emit(state.copyWith(formData: updatedFormData));
  }
  ...
  void _onUpdateMaxGuests(UpdateMaxGuestsEvent event, Emitter<RentCreateState> emit) {
    final updatedFormData = state.formData.copyWith(maximumNumberOfGuests: event.maxGuests);
    emit(state.copyWith(formData: updatedFormData));
  }

  void _onUpdateLivingRooms(UpdateLivingRoomsEvent event, Emitter<RentCreateState> emit) {
    final updatedFormData = state.formData.copyWith(livingRooms: event.livingRooms);
    emit(state.copyWith(formData: updatedFormData));
  }

  void _onUpdatePropertyDescription(UpdatePropertyDescriptionEvent event, Emitter<RentCreateState> emit) {
    final updatedFormData = state.formData.copyWith(propertyDescription: event.description);
    emit(state.copyWith(formData: updatedFormData));
  }

  void _onUpdateHouseRules(UpdateHouseRulesEvent event, Emitter<RentCreateState> emit) {
    final updatedFormData = state.formData.copyWith(houseRules: event.houseRules);
    emit(state.copyWith(formData: updatedFormData));
  }

  void _onUpdateImportantInfo(UpdateImportantInfoEvent event, Emitter<RentCreateState> emit) {
    final updatedFormData = state.formData.copyWith(importantInformation: event.importantInfo);
    emit(state.copyWith(formData: updatedFormData));
  }

  void _onUpdatePropertyFeatures(UpdatePropertyFeaturesEvent event, Emitter<RentCreateState> emit) {
    final updatedFormData = state.formData.copyWith(
  ...
  void _onUpdateGovernorate(UpdateGovernorateEvent event, Emitter<RentCreateState> emit) {
    final updatedFormData = state.formData.copyWith(
  ...
  void _onAddPhotos(AddPhotosEvent event, Emitter<RentCreateState> emit) {
    print('📱 BLoC: Received AddPhotosEvent with ${event.photos.length} photos');
    final updatedImages = List<File>.from(state.formData.selectedImages)..addAll(event.photos);
    final updatedFormData = state.formData.copyWith(selectedImages: updatedImages);
    print('📱 BLoC: Total images now: ${updatedImages.length}');
    
    // Emit the updated state with photos
    emit(state.copyWith(
      formData: updatedFormData,
      status: RentCreateStatus.photosAdded, // New status to indicate photos were added
    ));
  ...
  void _onSetPrimaryPhoto(SetPrimaryPhotoEvent event, Emitter<RentCreateState> emit) {
    final updatedFormData = state.formData.copyWith(primaryImageIndex: event.index);
    emit(state.copyWith(formData: updatedFormData));
  }
  ...
  void _onUpdateAddress(UpdateAddressEvent event, Emitter<RentCreateState> emit) {
  ...
  void _onUpdateStreet(UpdateStreetEvent event, Emitter<RentCreateState> emit) {
    final updatedFormData = state.formData.copyWith(streetAndBuildingNumber: event.street);
    emit(state.copyWith(formData: updatedFormData));
  }

  void _onUpdateLandMark(UpdateLandMarkEvent event, Emitter<RentCreateState> emit) {
    final updatedFormData = state.formData.copyWith(landMark: event.landMark);
    emit(state.copyWith(formData: updatedFormData));
  }

  void _onUpdateLocation(UpdateLocationEvent event, Emitter<RentCreateState> emit) {
    final updatedFormData = state.formData.copyWith(
  ...
  void _onUpdateAvailability(UpdateAvailabilityEvent event, Emitter<RentCreateState> emit) {
    final updatedAvailability = Map<DateTime, bool>.from(state.formData.dateAvailability);
    updatedAvailability[event.date] = event.isAvailable;
    final updatedFormData = state.formData.copyWith(dateAvailability: updatedAvailability);
    emit(state.copyWith(formData: updatedFormData));
  }

  void _onAddAvailableDate(AddAvailableDateEvent event, Emitter<RentCreateState> emit) {
  ...
  void _onRemoveAvailableDate(RemoveAvailableDateEvent event, Emitter<RentCreateState> emit) {
    final updatedDates = List<DateTime>.from(state.formData.availableDates);
  ...
  // API Event Handlers
  Future<void> _onCreatePropertyStepOne(CreatePropertyStepOneEvent event, Emitter<RentCreateState> emit) async {
    ...
    try {
      ...
      if (result.success) {
        final updatedFormData = state.formData.copyWith(propertyId: result.propertyId);
        emit(state.copyWith(
          isLoading: false,
          status: RentCreateStatus.stepOneCompleted,
          formData: updatedFormData,
          successMessage: result.message,
        ).updateStepStatus(0, isCompleted: true).updateStepStatus(1, isCompleted: true));
      } else {
        emit(state.copyWith(
          isLoading: false,
  ...
  Future<void> _onCreatePropertyStepTwo(CreatePropertyStepTwoEvent event, Emitter<RentCreateState> emit) async {
  ...
  Future<void> _onUploadImages(UploadImagesEvent event, Emitter<RentCreateState> emit) async {
    emit(state.copyWith(isLoading: true, status: RentCreateStatus.loading));
    ...
    try {
      ...
      if (result.success) {
        emit(state.copyWith(
          ...
          successMessage: result.message,
        ).updateStepStatus(2, isCompleted: true)); // Photos step
  ...
  Future<void> _onSetPrice(SetPriceEvent event, Emitter<RentCreateState> emit) async {
    emit(state.copyWith(isLoading: true, status: RentCreateStatus.loading));
    ...
    try {
      ...
      if (result.success) {
        emit(state.copyWith(
          ...
          successMessage: result.message,
        ).updateStepStatus(3, isCompleted: true)); // Price step
      } else {
        emit(state.copyWith(
          isLoading: false,
          status: RentCreateStatus.error,
  ...
  Future<void> _onAddAvailability(AddAvailabilityEvent event, Emitter<RentCreateState> emit) async {
    emit(state.copyWith(isLoading: true, status: RentCreateStatus.loading));
    ...
    try {
      ...
      if (result.success) {
        emit(state.copyWith(
          ...
          successMessage: result.message,
        ).updateStepStatus(4, isCompleted: true)); // Availability step
      } else {
        emit(state.copyWith(
          isLoading: false,
          status: RentCreateStatus.error,
  ...
  Future<void> _onSubmitProperty(SubmitPropertyEvent event, Emitter<RentCreateState> emit) async {
    emit(state.copyWith(isLoading: true, status: RentCreateStatus.loading));
  ...
  Future<void> _executePropertyCreationFlow(Emitter<RentCreateState> emit) async {
    ...
    if (state.formData.propertyId == null) {
      ...
      final updatedFormData = state.formData.copyWith(propertyId: stepOneResult.propertyId);
      emit(state.copyWith(formData: updatedFormData));
    }
    ...
    if (state.formData.availableDates.isNotEmpty) {
      ...
      if (!availabilityResult.success) throw Exception(availabilityResult.message);
    }
  }
